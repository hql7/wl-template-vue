...
import { localGet, localDel } from "@/library/js/storage"; // 导入存储函数
...
/**
 * 登记全局路由守卫
 * @param {*} verifyExpired 是否前端校验token过期
 */
function registerRouteGuard(verifyExpired) {
  router.beforeEach((to, from, next) => {
    ...
    if (local_auth && local_auth != 'undefined') {
      // 第一次打开页面token过期进入登陆页
      if (verifyExpired && isLoginExpired(local_auth)) {
...
// 登录及权限状态管理
function AUTH(verifyExpired) {
  monitoringVisibilityChange();
  registerRouteGuard(verifyExpired);
}
